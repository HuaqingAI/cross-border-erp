...
FR11: SKU 提供产品状态字段（上架/下架可售/下架不可售/临拓）用于业务标识、列表展示与基础编辑
...
| FR11 | Epic 4 | SKU 状态字段标识与基础展示/编辑 |
...
**Given** SKU 的客户质保期字段
**When** 产品部用户创建或编辑 SKU
**Then** 客户质保期始终继承并展示 SPU 当前值
**And** SKU 侧不可单独修改该字段

**Given** SPU 已有关联 SKU
**When** 产品部用户更新 SPU 的一级/二级/三级分类、供应商、禁止经营国家、客户质保期
**Then** 系统在同一事务内同步刷新关联 SKU 的继承字段镜像
...
**Sprint Change Note (2026-04-19)**

- 原 `Story 4.7: SKU 产品状态管理` 已取消
- 当前版本保留 SKU 状态字段、列表标签展示、筛选与表单基础编辑
- 不再承诺独立的状态管理流程、审计日志以及对下游“可售 SKU”范围的系统强管控
- 后续若业务需要更强治理能力，将以新的独立 Story 重新立项
...
管理员可通过 Excel 批量导入历史分类/SPU/SKU 数据（含完整校验反馈和错误修正机制），完成系统上线数据迁移；并可维护系统全局枚举值配置（单位、产品类型、证书类型等），并逐步将价格、资料、证书、FAQ、SPU、SKU 等模块接入统一枚举契约。
...
**Given** 国家/地区枚举已配置
**When** 财务部维护价格区域，或产品部维护资料适用国家/地区、SPU 禁止经营国家等字段
**Then** 国家/地区字段统一以下拉选择方式展示并提交标准编码

**Given** 枚举配置前端页面
**When** 管理员进入配置页
**Then** 左侧为枚举组列表（单位、产品类型、产品状态、包装类型、电参数、证书类型、FAQ问题类型、资料类型、币种、国家/地区）
...
**When** 用户进入证书列表页或新增/编辑页  
**Then** 证书类型筛选与表单字段均消费统一枚举选项  
**And** 详情页展示与列表筛选口径一致

**Given** `faq_question_type` 枚举组已配置  
**When** 用户进入 FAQ 列表页或新增/编辑页  
**Then** 问题类型筛选与表单字段均消费统一枚举选项  
**And** 详情页展示与列表筛选口径一致
